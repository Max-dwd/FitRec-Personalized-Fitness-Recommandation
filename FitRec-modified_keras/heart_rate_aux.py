...
from tensorflow.keras.layers import concatenate, add, dot, multiply
...
class keras_endoLSTM(object):
    def __init__(self, args, newModel):
        ...
        if newModel:
            ...
            self.model_save_base = base_dir / "fitrec" / "model_states"
            self.summaries_base = base_dir / "fitrec" / "logs"
            os.makedirs(self.model_save_base, exist_ok=True)
            os.makedirs(self.summaries_base, exist_ok=True)
            ...
            self.user_dim = args.attr_dim
            self.sport_dim = args.attr_dim
            self.gender_dim = args.attr_dim
            ...
            self.inputAtts = args.input_attributes.split(',')
    ...
    def build_model(self, args):
        ...
        # CLI flag is --out_reg, exposed as args.out_reg
        ...
        # compile model with modern tf.keras optimizer API
        optimizer = RMSprop(learning_rate=self.lr)
        model.compile(loss='mean_squared_error', optimizer=optimizer, metrics=['mae', root_mean_squared_error])
        ...
        if self.pretrain == True:
            ...
            pretrain_path = self.model_save_base / self.pretrain_model_file_name / (self.pretrain_model_file_name + "_bestValidScore")
            pretrain_model = keras.models.load_model(pretrain_path, custom_objects=custom_ob)
    ...
    def run_model(self, model):
        ...
        self.model_save_location = self.model_save_base / self.model_file_name
        self.summaries_dir = self.summaries_base / self.model_file_name
        os.makedirs(self.model_save_location, exist_ok=True)
        ...
        for iteration in range(1, self.max_epochs):
            ...
            model_save_fn = str(self.model_save_location / f"{self.model_file_name}_epoch_{iteration}")
            checkpoint = ModelCheckpoint(model_save_fn, verbose=1, save_best_only=False, save_weights_only=False, mode='auto', save_freq='epoch')

            # Keras 2 / TF 2: fit works directly with generators
            history = model.fit(trainDataGen, steps_per_epoch=train_steps_per_epoch, epochs=1, verbose=1, callbacks=[checkpoint])
            try:
                del history.model
            ...
            valid_score = model.evaluate(validDataGen, steps=valid_steps_per_epoch)
        ...
        # load best model
        custom_ob = {'root_mean_squared_error':root_mean_squared_error}
        best_model_path = self.model_save_location / f"{self.model_file_name}_epoch_{best_epoch}"
        best_model = keras.models.load_model(best_model_path, custom_objects=custom_ob)
        best_model.save(self.model_save_location / f"{self.model_file_name}_bestValidScore")
        ...
        test_score = best_model.evaluate(testDataGen, steps=test_steps_per_epoch)
