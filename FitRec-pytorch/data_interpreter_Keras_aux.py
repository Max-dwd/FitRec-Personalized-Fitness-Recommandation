...
class dataInterpreter(object):
    def __init__(self, inputAtts, targetAtts=['derived_speed'], includeUser=True, includeSport=False,
                 includeGender=False, includeTemporal=False, fn="endomondoHR_proper.json", scaleVals=True,
                 trimmed_workout_len=450, scaleTargets="scaleVals", trainValidTestSplit=[.8,.1,.1],
                 zMultiple=5, trainValidTestFN=None, max_workouts=None):
        self.filename = fn
        self.data_path = "./data"
        ...
        # Optional cap on total number of workouts (train+valid+test) to control memory usage
    ...
    def preprocess_data(self):
        ...
        # load index for train/valid/test (these are workout ids, and may be downsampled)
        self.loadTrainValidTest()

        if os.path.exists(self.processed_path):
            # preprocessed data already exist
            print("{} exists".format(self.processed_path))
            # allow_pickle for compatibility across numpy versions
            self.original_data = np.load(self.processed_path, allow_pickle=True)[0]
            # If max_workouts is set, prune original_data to only workouts actually used
            if self.max_workouts is not None and self.max_workouts > 0:
                used_ids = set(self.trainingSet) | set(self.validationSet) | set(self.testSet)
                for wid, (_, _, prev_list) in self.contextMap.items():
                    used_ids.add(wid)
                    for prev_wid in prev_list:
                        used_ids.add(prev_wid)
                pruned = [d for d in self.original_data if d.get('id') in used_ids]
                print("Pruned original_data from {} to {} workouts".format(len(self.original_data), len(pruned)))
                self.original_data = pruned
            self.map_workout_id()
        else:
            # not preprocessed yet, load raw data and preprocess
            # To reduce memory, only keep workouts that are actually used
            print("load original data (filtered to used workouts)")

            # Build the set of workout ids that are needed for train/valid/test and context
            used_ids = set(self.trainingSet) | set(self.validationSet) | set(self.testSet)
            for wid in self.contextMap:
                context = self.contextMap[wid]
                used_ids.add(wid)
                # context[2] is the list of previous workout ids
                for prev_wid in context[2]:
                    used_ids.add(prev_wid)
    ...
    def loadTrainValidTest(self):
        ...
        # Optionally downsample to limit memory usage
        if self.max_workouts is not None and self.max_workouts > 0:
            total = len(self.trainingSet) + len(self.validationSet) + len(self.testSet)
            if total > self.max_workouts:
                keep_ratio = float(self.max_workouts) / float(total)
                new_train = int(len(self.trainingSet) * keep_ratio)
                new_valid = int(len(self.validationSet) * keep_ratio)
                new_test = int(len(self.testSet) * keep_ratio)
                # Ensure we keep at least 1 element per split if the original split was non-empty
                if len(self.trainingSet) > 0 and new_train == 0:
                    new_train = 1
                if len(self.validationSet) > 0 and new_valid == 0:
                    new_valid = 1
                if len(self.testSet) > 0 and new_test == 0:
                    new_test = 1
                self.trainingSet = self.trainingSet[:new_train]
                self.validationSet = self.validationSet[:new_valid]
                self.testSet = self.testSet[:new_test]
                kept_ids = set(self.trainingSet) | set(self.validationSet) | set(self.testSet)
                # Prune contextMap to only contain entries for kept workouts and contexts that refer to kept workouts
                new_context = {}
                for wid, (since_last, since_begin, prev_list) in self.contextMap.items():
                    if wid not in kept_ids:
                        continue
                    new_prev = [p for p in prev_list if p in kept_ids]
                    if len(new_prev) == 0:
                        continue
                    new_context[wid] = (since_last, since_begin, new_prev)
                self.contextMap = new_context
                print("After subsampling, train/valid/test = {}/{}/{}".format(
    ...
    def scale_data(self, scaling=True): 
        ...
        # write to disk: store a 1-element object array so we can reload with allow_pickle=True
        np.save(self.processed_path, np.array([self.original_data], dtype=object))
