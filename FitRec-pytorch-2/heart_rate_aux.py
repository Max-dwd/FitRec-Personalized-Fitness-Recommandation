...
def main(args: argparse.Namespace):
    # Match original defaults but use a proper list of attribute names
    input_attrs = [s.strip() for s in args.input_attributes.split(",") if s.strip()]

    cfg = build_config_from_args(
        args=args,
        target_attr="heart_rate",
        input_attrs=input_attrs,
        # Original heart_rate_aux.py does NOT apply dropout on context LSTMs
